...
def run(
    ...
    # I am using the local ckpt.t7 so it's easier to locate
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                # change the normal array to tensor array
                xywhs = torch.Tensor(bbox_xywh)
                confss = torch.Tensor(confs)
                
                # yolov5 class for making visualization box
                annotator = Annotator(im0, line_width=line_thickness, example=str(names))
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    pixel = 10   # add 10 pixel, u can cuztomize it
                    xyxy[1] -= pixel   # reduce the y1 with the pixel
                    xyxy[3] += pixel   # add the y2 with the pixel
                ...
                if len(outputs) > 0:
                    bbox_xyxy = outputs[:, :4]   # DeepSORT bounding box
                    identities = outputs[:, -1]   # DeepSORT track_id / unique_id in array shape
